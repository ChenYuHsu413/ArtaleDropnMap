> **⚠️ DEPRECATED：僅歷史紀錄。** 依 Artale-only 資料驗證原則，跨版本（GMS/韓服等）數值比對**不作為驗證依據**，也不得用於填值。本文保留僅供了解當初查證脈絡；文中「孤證解除」「首發官方值」等結論**已作廢**，Artale 數值一律以 Artale 自身來源為準（見 README 資料來源章節）。
