...
| `data-src/map_gms_ids.json` | **人工權威地圖配對層**：中文完整圖名 → GMS map id，**於 monster-vote 之後、patch 之前**套用，修正投票把不同圖壓到同一 GMS id 的錯配。信心分級：`verified`(名稱+怪物雙訊號一致)／`name-high`(名稱權威，如編號/方位；同怪系列怪證退化時採用)／`manual`(人工) **覆蓋投票並重算所有 GMS 衍生欄位(minimap/gms_name/gms_neighbors，不殘留舊 id)**；`region-exclusive`(如台北101 等台灣獨佔圖) **跳過 GMS 配對、標 `region_exclusive` 而非配對失敗**；`needs-review` **忽略**(維持投票，僅存候選 id/依據供人工複查)。報告見 `data_gaps.map_gms_override`；半自動比對法與稽核明細見 `docs/map_gms_ids_pilot_audit.md` |
